...
class App extends React.Component {
  ...
  handleChange = (e) => {
    ...
    this.state.data.map((row) => {
      ...
      const regex = new RegExp(
        `.*?${escapeStringRegexp(e.target.value)}.*?`,
        "i"
      );
  ...
  editRow = (row) => {
    ...
    const newArray = this.state.data.map((personaje) =>
      personaje.id === row.id
        ? { ...personaje, id: id, name: name, species: species }
        : personaje
    );
    this.setState({
      data: newArray,
    ...
    return (
      <React.Fragment>
        ...
        <nav className="navbar navbar-expand-lg ">
          <button className="btn btn-info" onClick={this.obtenerPersonajes}>
            Listar personajes
          </button>
